...
class MultiInputNN(L.LightningModule):
    def __init__(self, src_vocab_size, d_model, num_heads, num_layers, d_ff, max_seq_len, num_output_nodes, dropout=0.0, mean_pool=True, lr=0.00001, mlp_input_size=12, mlp_hidden_size=128):
        ...
        self.fc = nn.Linear(d_model + mlp_hidden_size, num_output_nodes)
    ...
    def forward(self, src, mlp):
        ...
        mlp_output = self.mlp(mlp)

        combined_output = torch.cat((pooled_output, mlp_output), dim=1)
        
        return self.fc(combined_output)

    def training_step(self, batch, batch_idx):
        logits = self.forward(batch['input_ids'], batch['mlp_input'])
...
checkpoint_callback = ModelCheckpoint(
    dirpath='/home/dzhou/multi_input_checkpoints',
    filename='multi-{epoch:02d}-{val_acc:.2f}',
...
logger = CSVLogger('csv_logs', name='multi_csv_logs')
...
trainer = L.Trainer(
    ...
    callbacks=[checkpoint_callback] #, EarlyStopping(monitor='val_acc', mode = 'max', min_delta=0.001, patience=50)]
)
